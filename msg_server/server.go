...
func (self *MsgServer) parseProtocol(cmd protocol.CmdSimple, session *libnet.Session) error {
	...
	// //需要登录的接口在没有登陆的情况下立即断开连接
	// if cmdName == protocol.SEND_CLIENT_ID_CMD ||
	// 	cmdName == protocol.SUBSCRIBE_CHANNEL_CMD ||
	// 	cmdName == protocol.ROUTE_MESSAGE_P2P_CMD ||
	// 	cmdName == protocol.ROUTE_MESSAGE_TOPIC_CMD ||
	// 	cmdName == protocol.ROUTE_CHANGE_MESSAGE_SERVER_CMD ||
	// 	cmdName == protocol.ROUTE_ASK_CMD {
	// } else {
	// 	if session.State == nil {
	// 		log.Error("error:", err)
	// 		log.Error("CMDNAME: ", cmdName)
	// 		session.Close()
	// 		return err
	// 	}
	// }
	...
	case protocol.ROUTE_CHANGE_MESSAGE_SERVER_CMD:
		err = pp.procRouterChangeMsgServer(&cmd, session)
		if err != nil {
			log.Error("error:", err)
			return err
		}
	...
	case protocol.ROUTE_ASK_CMD:
		err = pp.procRouteAsk(&cmd, session)
		if err != nil {
			log.Error("error:", err)
			return err
		}
