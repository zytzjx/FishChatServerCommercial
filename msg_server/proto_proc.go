...
func (self *ProtoProc) procPing(cmd protocol.Cmd, session *libnet.Session) error {
	...
	self.msgServer.sessions[cid].State.(*base.SessionState).LastPing = time.Now().Unix()
...
func (self *ProtoProc) procClientID(cmd protocol.Cmd, session *libnet.Session) error {
	...
	self.msgServer.sessions[ClientID].State = base.NewSessionState(ClientID, time.Now().Unix())

	//获取用户未读信息
	go self.procOfflineMsg(session, ClientID)

	// 广播消息通知其好友
	go self.broadcastToFriends(ClientID, session, true)
...
//接收用户登录Token
func (self *ProtoProc) procToken(cmd protocol.Cmd, session *libnet.Session) error {
	log.Info("procToken")
	var (
		err       error
		tokenData GetTokenTemplate
		ClientID  string
		Platform  string
		resp      *http.Response
		temp      []byte
	...
	if len(cmd.GetArgs()) < protocol.SEND_CLIENT_ID_CMD_ARGS_NUM {
		log.Info(info.NOT_ENOUGH_ARGUMENTS)
		self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), false, info.NOT_ENOUGH_ARGUMENTS)
		self.clientQuit(session)
		return nil
	}

	token := cmd.GetArgs()[0]
	resp, err = http.Get(self.msgServer.cfg.VerifyTokenServer + self.msgServer.cfg.VerifyTokenUrl + "?IMToken=" + token)
	if err != nil {
		log.Error("Error:", err)
		self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), false, info.UNABLE_TO_ACCESS_THE_AUTH_SERVER)
		self.clientQuit(session)
		return err
	}
	...
	if err != nil {
		log.Error("Error:", err)
		self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), false, info.FAILED_TO_PARSE_DATA)
		self.clientQuit(session)
		return err
	}

	err = json.Unmarshal(temp, &tokenData)
	if err != nil {
		log.Error("Error:", err)
		self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), false, info.FAILED_TO_PARSE_DATA)
		self.clientQuit(session)
	...
	if tokenData.Status == "0" {
		...
		self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), false, info.INVALID_TOKEN)
		self.clientQuit(session)
		return err
	}
	...
	if clientInfo != nil {
		if clientInfo.Alive == true {
			...
			if clientInfo.MsgServerAddr != self.msgServer.cfg.LocalIP {
				...
				if err != nil {
					log.Error(err.Error())
					self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), false, info.ERROR)
					return err
				}
		...
		sessionStoreData := mongo_store.SessionStoreData{ClientID, session.Conn().RemoteAddr().String(),
			self.msgServer.cfg.LocalIP, clientInfo.Friends, true, Platform}
		...
		sessionStoreData := mongo_store.SessionStoreData{ClientID, session.Conn().RemoteAddr().String(),
			self.msgServer.cfg.LocalIP, []string{}, true, Platform}
	...
	session.EnableAsyncSend(10)
	self.respCmd(protocol.RESP_TOKEN_CMD, session, cmd.GetReport(), true, "")
	return err
}
...
func (self *ProtoProc) procLogout(cmd protocol.Cmd, session *libnet.Session) error {
	...
	if clientID != "" {
		// 标记用户离线
		_, err := self.msgServer.mongoStore.UpdateSessionAlive(mongo_store.DATA_BASE_NAME, mongo_store.CLIENT_INFO_COLLECTION, clientID, self.msgServer.cfg.LocalIP, false)
		if err != nil {
			log.Error(err.Error())
	...
	// 广播消息通知其好友
	go self.broadcastToFriends(clientID, session, false)
...
func (self *ProtoProc) procSendMessageP2P(cmd protocol.Cmd, session *libnet.Session) error {
	...
	if self.msgServer.sessions[send2ID] != nil {
		...
		if self.msgServer.channels[protocol.SYSCTRL_SEND] != nil {
			...
			if storeSession.Alive == false && storeSession.Platform == "ios" {
				// resp, err := http.Post(
				// 	self.msgServer.cfg.PushServer+self.msgServer.cfg.PushUrl,
				// 	"application/x-www-form-urlencoded",
				// 	strings.NewReader("userId="+send2ID+"&message="+send2Msg))
				// if err != nil {
				// 	log.Error("Error:", err)
				// 	return err
				// }
				...
				send2IDMsgNum := self.msgServer.mongoStore.ReadP2PRecordNumber(mongo_store.DATA_BASE_NAME,
					mongo_store.CLIENT_INFO_COLLECTION, send2ID)

				addr := self.msgServer.cfg.PushServer + self.msgServer.cfg.PushUrl
				reqPost, err := http.NewRequest("POST", addr,
					strings.NewReader("userId="+send2ID+"&message="+send2Msg+"&msgNum="+strconv.Itoa(send2IDMsgNum)))
				if err != nil {
					log.Error(err.Error())
					self.respCmd(NCommendMappedMap[msgType].RespCmd, session, cmd.GetReport(), false, info.UNABLE_TO_ACCESS_THE_PUSH_SERVER)
					return err
				}

				reqPost.Header.Set("Content-Type", "application/x-www-form-urlencoded")
				reqPost.Header.Set("Connection", "close")
				...
				resp, err := defaultClient.Do(reqPost)
				if err != nil {
					log.Error(err.Error())
					self.respCmd(NCommendMappedMap[msgType].RespCmd, session, cmd.GetReport(), false, info.UNABLE_TO_ACCESS_THE_PUSH_SERVER)
					return err
				...
				if resp.StatusCode == http.StatusOK {
					...
					self.respCmd(NCommendMappedMap[msgType].RespCmd, session, cmd.GetReport(), false,
						info.PUSH_SERVER_ERROR+strconv.Itoa(resp.StatusCode))
					return err
				}
...
func (self *ProtoProc) procLeaveTopic(cmd protocol.Cmd, session *libnet.Session) error {
	...
	//群组成员为空时移除群组
	if len(users) == 0 {
		err = self.msgServer.mongoStore.RemoveTopicsFromTopicId(mongo_store.DATA_BASE_NAME, mongo_store.TOPIC_INFO_COLLECTION, topicId)
		if err != nil {
			log.Error(err.Error())
			self.respCmd(protocol.RESP_LEAVE_TOPIC_CMD, session, cmd.GetReport(), false, info.ERROR)
			return err
...
//广播自己的状态给好友
func (self *ProtoProc) broadcastToFriends(cid string, session *libnet.Session, alive bool) {
	CidData, err := self.msgServer.mongoStore.GetClientFromId(mongo_store.DATA_BASE_NAME, mongo_store.CLIENT_INFO_COLLECTION, cid)
	if err != nil {
		log.Error(err.Error())
		return
	}

	friendList := CidData.Friends
	msgResult := self.msgServer.mongoStore.GetOnlineClientsFromIds(mongo_store.DATA_BASE_NAME, mongo_store.CLIENT_INFO_COLLECTION, friendList)

	if msgResult != nil {
		var status int
		if alive == true {
			status = protocol.CLIENT_NOTIFY_FRIEND_ONLNE
		} else {
			status = protocol.CLIENT_NOTIFY_FRIEND_OFFLINE
		}
		notifyMsg := protocol.NewClientNotifyMsg(status, cid)
		...
		for _, v := range msgResult {
			...
			if self.msgServer.sessions[v.ClientID] != nil {
				receive := protocol.NewCmdResponse(protocol.RESP_PUSH_P2P_CMD)
				receive.AddArg(string(msg))
				receive.AddArg(v.ClientID)

				if self.msgServer.sessions[v.ClientID] != nil {
					self.msgServer.sessions[v.ClientID].Send(receive)
					if err != nil {
						log.Error(err.Error())
						return
					}
				...
				//Router
				if self.msgServer.channels[protocol.SYSCTRL_SEND] != nil {
					rcmd := protocol.NewCmdSimple(protocol.ROUTE_PUSH_P2P_CMD)
					rcmd.AddArg(string(msg))
					rcmd.AddArg(v.ClientID)
					...
					routerMsg := protocol.NewCmdSimple(protocol.ROUTE_MSG_CMD)
					routerMsg.AddArg(v.MsgServerAddr)
					routerMsg.AddArg(string(temp))
...
//获取在线好友信息
func (self *ProtoProc) procClientOnlineStatus(cmd protocol.Cmd, session *libnet.Session) error {
	log.Info("procClientOnlineStatus")
	var err error

	if session.State == nil {
		self.respCmd(protocol.RESP_CLIENT_ONLINE_STATUS, session, cmd.GetReport(), false, info.YOU_HAVE_NOT_LANDED)
		return nil
	}

	if len(cmd.GetArgs()) < protocol.SEND_CLIENT_ONLINE_STATUS_ARGS_NUM {
		log.Info(info.NOT_ENOUGH_ARGUMENTS)
		self.respCmd(protocol.RESP_CLIENT_ONLINE_STATUS, session, cmd.GetReport(), false, info.NOT_ENOUGH_ARGUMENTS)
		return nil
	}
	...
	if friendId != "" {
		// 标记用户离线
		clientInfo, err := self.msgServer.mongoStore.GetClientFromId(mongo_store.DATA_BASE_NAME, mongo_store.CLIENT_INFO_COLLECTION, friendId)
		if err != nil {
			log.Error(err.Error())
			self.respCmd(protocol.RESP_CLIENT_ONLINE_STATUS, session, cmd.GetReport(), false, info.ERROR)
			return err
		}
		...
		notifyMsg := protocol.NewClientNotifyMsg(status, friendId)
		...
		//定义返回用户请求信息
		resp := protocol.NewCmdResponse(protocol.RESP_CLIENT_ONLINE_STATUS)
		resp.Repo = cmd.GetReport()
		resp.Time = time.Now().Unix()
		resp.AddArg(string(msg))
		//返回用户请求
		err = session.Send(resp)
		if err != nil {
			log.Error("report :", err.Error())
