...
func (self *ProtoProc) procRouteMsg(cmd protocol.Cmd, session *libnet.Session) error {
	...
	case protocol.ROUTE_PUSH_P2P_CMD:
		err = self.procRoutePushP2P(msg, session)
		if err != nil {
			log.Error("error:", err)
			return err
		}
...
func (self *ProtoProc) procRoutePushP2P(cmd protocol.Cmd, session *libnet.Session) error {
	log.Info("procRoutePushP2P")
	var err error

	if len(cmd.GetArgs()) < protocol.ROUTE_PUSH_P2P_CMD_ARGS_NUM {
		log.Info(info.NOT_ENOUGH_ARGUMENTS)
		return nil
	}

	receive:= protocol.NewCmdResponse(protocol.RESP_PUSH_P2P_CMD)
	receive.AddArg(cmd.GetArgs()[0])
	receive.AddArg(cmd.GetArgs()[1])

	if self.msgServer.sessions[cmd.GetArgs()[1]] != nil {
		self.msgServer.sessions[cmd.GetArgs()[1]].Send(receive)
		if err != nil {
			log.Error(err.Error())
			return err
		}
