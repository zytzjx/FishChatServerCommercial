...
func userConnnectServer(i int, answers []string, cfg Config) {
	...
	go func() {
		for {
			...
			smsg.AddArg("{\"fromUser\":\"1286\",\"message\":\"{toUserid:'1090',text:'提提哦'}\",\"toUser\":\"bd\",\"type\":\"TXT\"}")
			smsg.AddArg("bd")
			...
			rt := time.Duration(rand.Int63n(60000000))
			// log.Info(rt)
			time.Sleep(rt)
		}
	}()
...
func main() {
	...
	go userConnnectServer(0, answers, cfg)
