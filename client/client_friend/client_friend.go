...
func main() {
	...
	for {
		//获取操作
		fmt.Print("Command (add,newadd,del,list,logout,alive,token) : ")
		if _, err = fmt.Scanf("%s\n", &input); err != nil {
			log.Error(err.Error())
		}
		...
		case "alive":
			smsg = protocol.NewCmdSimple(protocol.SEND_CLIENT_ONLINE_STATUS)
			fmt.Print("Please input friend name : ")
			if _, err = fmt.Scanf("%s\n", &input); err != nil {
				log.Error(err.Error())
			}
		...
		case "token":
			smsg = protocol.NewCmdSimple(protocol.SEND_GET_TOKEN)
			//SEND_GET_TOKEN APP_NAME, RES_TYPE, ACTION_TYPE, X_Y
			smsg.AddArg("diiia")
			smsg.AddArg("image")
