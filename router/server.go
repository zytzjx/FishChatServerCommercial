...
func (self *Router) connectDisconnectedMsgServerList() {
	for _, ms := range self.disConnectedMsgServerList {
		...
		if err == nil {
			...
			go self.handleMsgServerClient(msgServerClient)
		}
	}
}
		...
		if err == nil {
			...
			go func() {
				for {
					...
					err := self.handleBrotherServerClient(msg, brotherServerClient)
					if err != nil {
						log.Error(err.Error())
					}
				}
...
//获取Brother管理的MsgServer列表，并更新到数据库
...
func (self *Router) handleMsgServerClient(msc *libnet.Session) {
	...
	for {
		...
		case protocol.SEND_CHANGE_MESSAGE_SERVER_CMD:
			err := pp.procChangeMessageServer(&msg, msc)
			if err != nil {
				log.Warning(err.Error())
			}
		case protocol.SEND_ASK_CMD:
			err := pp.procSendAskMsg(&msg, msc)
			if err != nil {
				log.Warning(err.Error())
			}
...
//解析Brother过来的命令
func (self *Router) handleBrotherServerClient(msg protocol.CmdSimple, rsc *libnet.Session) error {
	var err error
	...
	case protocol.SUBSCRIBE_CHANNEL_CMD:
		err := pp.procSubscribeChannel(&msg, rsc)
		if err != nil {
			log.Error(err.Error())
			return err
		}
	case protocol.SEND_MESSAGE_P2P_CMD:
		err := pp.procSendMsgP2P(&msg, rsc)
		if err != nil {
			log.Error(err.Error())
			return err
		}
	case protocol.SEND_MESSAGE_TOPIC_CMD:
		err := pp.procSendMsgTopic(&msg, rsc)
		if err != nil {
			log.Error(err.Error())
			return err
		}
	case protocol.SEND_CHANGE_MESSAGE_SERVER_CMD:
		err := pp.procChangeMessageServer(&msg, rsc)
		if err != nil {
			log.Error(err.Error())
			return err
		}
	case protocol.SEND_ASK_CMD:
		err := pp.procSendAskMsg(&msg, rsc)
		if err != nil {
			log.Error(err.Error())
			return err
		}
...
func (self *Router) subscribeChannels() error {
	...
	kvStoreData := mongo_store.KVData{"routerManageMsgServer", self.cfg.LocalIP, self.cfg.MsgServerList}
